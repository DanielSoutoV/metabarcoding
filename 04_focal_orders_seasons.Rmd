...
There must be certainly a cleaner way how to do this without so many steps, butI then had to change the classification rank names again (rank_0 instead of Kingdom) as this is more friendly with MicroBiotaProcess. Within the metabarcoding package community, there seems to have been several issues with the ranks, some have root, some dont, some have subfamily, some dont, so it seems easier to simply keep rank_0 to _7 in this case and take note of the nomenclature you are using. I am using Kingdom;Phylum;Clss;Order;Family;SubFamily;genus:species.
...
colnames(tax_table(ps_leps)) <- c("rank_0", "rank_1", "rank_2", "rank_3", "rank_4", "rank_5", "rank_6" ,"rank_7")
...
colnames(tax_table(ps_coleo)) <- c("rank_0", "rank_1", "rank_2", "rank_3", "rank_4", "rank_5", "rank_6" ,"rank_7")
...
colnames(tax_table(ps_dips)) <- c("rank_0", "rank_1", "rank_2", "rank_3", "rank_4", "rank_5", "rank_6" ,"rank_7")
...
colnames(tax_table(ps_bees)) <- c("rank_0", "rank_1", "rank_2", "rank_3", "rank_4", "rank_5", "rank_6" ,"rank_7")
...
colnames(tax_table(ps_blats)) <- c("rank_0", "rank_1", "rank_2", "rank_3", "rank_4", "rank_5", "rank_6" ,"rank_7")
...
colnames(tax_table(ps_hemi)) <- c("rank_0", "rank_1", "rank_2", "rank_3", "rank_4", "rank_5", "rank_6" ,"rank_7")

```

Ok we now have datasets separated for each order (ForestGEO focal plus diptera - if we want more, we can adapt the script above)
...
diffclade_bees <- ggdiffclade(
  ...
  layout="radial",
  cladetext = 0.7,
  taxlevel=1, #taxonomy level from 1 to 8 kingdome:phylum:class:order:family:subfamily:genus:species
  removeUnkown=TRUE,
  reduce=TRUE # This argument is to remove the branch of unknown taxonomy.
) +
  scale_fill_manual(
...
ps_bees %>% as.MPSE() %>% mp_rrarefy() %>% mp_diff_analysis(.abundance=RareAbundance, .group=SEASON, action='get') %>% dplyr::filter(grepl("^s_", f)) %>% ggdiffbox(colorlist=c("darkcyan", "orange"), notch = FALSE)
