...
obj <- readRDS('data/correct_anal/taxmap_object.rds') #loads the taxmap object created in script 02_metacoder_heat_trees
...
classtaxa_leps <- get_taxadf(obj=ps_leps, taxlevel=6) #taxonomy level from 1 to 7 kingdom:phylum:class:order:family:genus:species
...
ps_leps %>% as.MPSE() %>% mp_rrarefy() %>% mp_diff_analysis(.abundance=RareAbundance, .group=SEASON, action='get') %>% dplyr::filter(grepl("^f_", f)) %>% ggdiffbox(colorlist=c("steelblue", "goldenrod"), notch = FALSE) -> ggdiffbox_leps
...
obj %>%  metacoder::filter_taxa(taxon_names %in% c("Erebidae", "Crambidae", "Pyralidae"),#here is to fliter the figure by groups
              subtaxa = TRUE) -> leps_fam #we will create separate files for each order as this simplifies downstream analysis in microbiotaprocess - until I find a way to filter taxa on the mpse object directly.

ps_leps_fam <- metacoder::as_phyloseq(leps_fam,
                      otu_table = "tax_data",
                      otu_id_col = "bin_uri",
                      sample_data = sample,
                      sample_id_col = "sampleID") %>% microViz::tax_mutate(rank_0 = "Animalia",rank_01 = "Arthropoda", rank_02 = "Insecta", rank_03 = "Lepidoptera", .before = 1) -> ps_leps_fam 
colnames(tax_table(ps_leps_fam)) <- c("k__", "p__", "c__", "o__", "f__", "g__", "s__")

ps_leps_fam %>% as.MPSE() %>% mp_rrarefy() %>% mp_diff_analysis(.abundance=RareAbundance, .group=SEASON, action='get') %>% dplyr::filter(grepl("^g_", f)) %>% ggdiffbox(colorlist=c("steelblue", "goldenrod"), notch = FALSE) -> ggdiffbox_leps_fam

pdf("./04_focal_orders_seasons_files/leps_fam_diffbox.pdf") 
ggdiffbox_leps_fam
dev.off()
...
pdf("./04_focal_orders_seasons_files/coleo_ggdiffbox.pdf")
coleo_ggdiffbox
dev.off()
...
obj %>%  metacoder::filter_taxa(taxon_names %in% c("Curculionidae", "Staphylinidae", "Scarabaeidae", "Cantharidae"),#here is to fliter the figure by groups
              subtaxa = TRUE) -> coleo_fam #we will create separate files for each order as this simplifies downstream analysis in microbiotaprocess - until I find a way to filter taxa on the mpse object directly.

ps_coleo_fam <- metacoder::as_phyloseq(coleo_fam,
                      otu_table = "tax_data",
                      otu_id_col = "bin_uri",
                      sample_data = sample,
                      sample_id_col = "sampleID") %>% microViz::tax_mutate(rank_0 = "Animalia",rank_01 = "Arthropoda", rank_02 = "Insecta", rank_03 = "Coleoptera", .before = 1) -> ps_coleo_fam 
colnames(tax_table(ps_coleo_fam)) <- c("k__", "p__", "c__", "o__", "f__", "g__", "s__")

ps_coleo_fam %>% as.MPSE() %>% mp_rrarefy() %>% mp_diff_analysis(.abundance=RareAbundance, .group=SEASON, action='get') %>% dplyr::filter(grepl("^g_", f)) %>% ggdiffbox(colorlist=c("steelblue", "goldenrod"), notch = FALSE) -> ggdiffbox_coleo_fam

pdf("./04_focal_orders_seasons_files/coleo_fam_diffbox.pdf") 
ggdiffbox_coleo_fam
dev.off()
...
ps_dips %>% as.MPSE() %>% mp_rrarefy() %>% mp_diff_analysis(.abundance=RareAbundance, .group=SEASON, action='get') %>% dplyr::filter(grepl("^f_", f)) %>% ggdiffbox(colorlist=c("steelblue", "goldenrod"), notch = FALSE) -> diptera_ggdiffbox
...
obj %>%  metacoder::filter_taxa(taxon_names %in% c("Milichiidae", "Cecidomyiidae", "Culicidae", "Sciaridae"),#here is to fliter the figure by groups
              subtaxa = TRUE) -> diptera_fam #we will create separate files for each order as this simplifies downstream analysis in microbiotaprocess - until I find a way to filter taxa on the mpse object directly.

ps_diptera_fam <- metacoder::as_phyloseq(diptera_fam,
                      otu_table = "tax_data",
                      otu_id_col = "bin_uri",
                      sample_data = sample,
                      sample_id_col = "sampleID") %>% microViz::tax_mutate(rank_0 = "Animalia",rank_01 = "Arthropoda", rank_02 = "Insecta", rank_03 = "dipteraptera", .before = 1) -> ps_diptera_fam 
colnames(tax_table(ps_diptera_fam)) <- c("k__", "p__", "c__", "o__", "f__", "g__", "s__")

ps_diptera_fam %>% as.MPSE() %>% mp_rrarefy() %>% mp_diff_analysis(.abundance=RareAbundance, .group=SEASON, action='get') %>% dplyr::filter(grepl("^f_", f)) %>% ggdiffbox(colorlist=c("steelblue", "goldenrod"), notch = FALSE) -> ggdiffbox_diptera_fam

pdf("./04_focal_orders_seasons_files/diptera_fam_diffbox.pdf") 
ggdiffbox_diptera_fam
dev.off()
...
diffclade_hemi <- ggdiffclade(
  ...
  scale_fill_manual(
    values=c("goldenrod", "steelblue" ) #Note that I had to change the order here, there are no hemiptera differences in DRY season so they did not come out in plot
  ) +
  guides(color = guide_legend(
    keywidth = 0.1, 
    keyheight = 0.6,
...
ps_hemi %>% as.MPSE() %>% mp_rrarefy() %>% mp_diff_analysis(.abundance=RareAbundance, .group=SEASON, action='get') %>% dplyr::filter(grepl("^f_", f)) %>% ggdiffbox(colorlist=c("steelblue", "goldenrod"), notch = FALSE) -> hemiptera_ggdiffbox
...
es_hemi <- ggeffectsize(obj=dereshemi, 
                     lineheight=0.1,
                     linewidth=0.3, ytextsize = 0.3) + 
  scale_color_manual(values=c( "goldenrod", "steelblue")) #same change due to wrong plotting 
...
pdf("./04_focal_orders_seasons_files/bees_cladogram.pdf")
diffclade_bees
dev.off()


ps_bees %>% as.MPSE() %>% mp_rrarefy() %>% mp_diff_analysis(.abundance=RareAbundance, .group=SEASON, action='get') %>% dplyr::filter(grepl("^f_", f)) %>% ggdiffbox(colorlist=c("steelblue", "goldenrod"), notch = FALSE) -> bees_diffbox

pdf("./04_focal_orders_seasons_files/bees_diffbox.pdf")
bees_diffbox
dev.off()
...
pdf("./04_focal_orders_seasons_files/bees_effectsize.pdf")
es_bees
dev.off()
...
pdf("./04_focal_orders_seasons_files/blats_cladogram.pdf")
diffclade_blats
dev.off()


#ps_blats %>% as.MPSE() %>% mp_rrarefy() %>% mp_diff_analysis(.abundance=RareAbundance, .group=SEASON, action='get') %>% dplyr::filter(grepl("^st_", f)) %>% ggdiffbox(colorlist=c("steelblue", "goldenrod"), notch = FALSE) -> blatts_diffbox
...
pdf("./04_focal_orders_seasons_files/blats_effectsize.pdf")
es_blats
dev.off()
```