...
tradmetabrparallel <- read.csv('data/correct_anal/tradmetabrparallel_final_reps.csv') #the file "_reps"is the same as tradmetabrparallel_final excep I copy pasted the 'prarallel' samples to keep as traditional so they would show up in the venn diagram which uses ALL traditional samples. Otehrwise it treats them as a separate sampling procedure.
sample <- read.csv('data/correct_anal/metadata_trad_metabr_parallel.csv') #this includes info on location/collection method/classification method and any more info can be added
...
obj %>% metacoder::filter_taxa(taxon_names %in% c("Notodontidae"), subtaxa = TRUE) -> noto_LT
...
obj %>% metacoder::filter_taxa(taxon_names %in% c("Lycaenidae"), subtaxa = TRUE) -> lycan_LT
...
obj %>% metacoder::filter_taxa(taxon_names %in% c("Apidae", "Halictidae"), subtaxa = TRUE) -> bees_LT
...
obj %>% metacoder::filter_taxa(taxon_names %in% c("Limacodidae", "Nolidae", "Riodinidae", "Megalopygidae", "Elachistidae", "Sphingidae","Laciocampidae","Papilionidae","Autostichidae", "Megalopygidae","Mimallonidae"), subtaxa = TRUE) -> extra_LT
...
noto_LT %>%
  heat_tree(node_label = taxon_names,
            ...
            layout = "davidson-harel", # The primary layout algorithm
            initial_layout = "reingold-tilford",
            output_file = "./07_trad_metabar_parallel_data_files/notodontidae_heattree.pdf") # The layout algorithm that initializes node locations
...
lycan_LT %>%
  heat_tree(node_label = taxon_names,
            ...
            layout = "davidson-harel", # The primary layout algorithm
            initial_layout = "reingold-tilford",
            output_file = "./07_trad_metabar_parallel_data_files/lycaenidae_heattree.pdf") # The layout algorithm that initializes node locations
...
extra_LT %>%
  heat_tree(node_label = taxon_names,
            ...
            layout = "davidson-harel", # The primary layout algorithm
            initial_layout = "reingold-tilford",
            output_file = "./07_trad_metabar_parallel_data_files/extras_heattree.pdf") # The layout algorithm that initializes node locations
...
ggsave("./07_trad_metabar_parallel_data_files/venn_notodontidae.pdf", vennp_noto)

dev.off()
grid::grid.draw(vennp_lycan)
ggsave("./07_trad_metabar_parallel_data_files/venn_luycaenidae.pdf", vennp_lycan)
...
ggsave("./07_trad_metabar_parallel_data_files/venn_extra.pdf", vennp_extra)


```

```{r}
#this is to make a venn diagram of three classification methods (parllel, metabarcoding and forestgeo data)
...
vennp_all <- venn.diagram(vennlist_all,
                      ...
                      filename=NULL,
                      disable.logging = TRUE,
                      fill=c("darkolivegreen3", "darkorchid", "firebrick4"),
                      cat.col=c("darkolivegreen3", "darkorchid", "firebrick4"),
                      alpha = 0.75, 
                      fontfamily = "serif",
                      fontface = "bold",
                      cex = 1.2,
...
ggsave("./07_trad_metabar_parallel_data_files/venn_trad_meta_parallel.pdf", vennp_all)
...
```{r}
# This section is just to have a general metabarcoding vs parallel analysis (venn diagram and heat tree)

tradmetabrparallel <- read.csv('data/final_anal/tradmetabrparallel_final_reps.csv') #the file "_reps"is the same as tradmetabrparallel_final excep I copy pasted the 'prarallel' samples to keep as traditional so they would show up in the venn diagram which uses ALL traditional samples. Otehrwise it treats them as a separate sampling procedure.
...
ps_obj <- metacoder::as_phyloseq(obj,
                      ...
                      sample_data = sample_parallel,
                      sample_id_col = "sampleID")
#%>% microViz::tax_mutate(rank_0 = "Animalia",rank_01 = "Arthropoda", rank_02 = "Insecta", .before = 1) -> ps_obj
#colnames(tax_table(ps_obj)) <- c("k__", "p__", "c__", "o__", "f__", "g__", "s__")

phyloseq_validate(ps_obj, remove_undetected = TRUE, verbose = TRUE)


vennlist_parallel <- get_vennlist(obj=ps_obj, factorNames="methodClass")
vennp_parallel <- venn.diagram(vennlist_parallel,
                      height=5,
                      width=5, 
                      filename=NULL,
                      disable.logging = TRUE,
...
dev.off()
grid::grid.draw(vennp_parallel)
ggsave("./07_trad_metabar_parallel_data_files/venn_parallel.pdf", vennp_parallel)
...
heat_tree(node_label = taxon_names,
          ...
          layout = "davidson-harel", # The primary layout algorithm
          initial_layout = "reingold-tilford",
          output_file = "./07_trad_metabar_parallel_data_files/parallel_heattree.pdf")
