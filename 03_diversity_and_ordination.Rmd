...
```{r}
ps_obj %>% as.MPSE() %>% mp_rrarefy() %>% mp_diff_analysis(.abundance=RareAbundance, .group=SEASON, action='get') %>% dplyr::filter(grepl("^c__", f)) %>% ggdiffbox(colorlist=c("orange", "darkcyan"))
```
