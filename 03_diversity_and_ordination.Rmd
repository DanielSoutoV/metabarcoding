...
```
Note now that it is 'taxmap_object_binary.rds' - we have transformed all data to a binary presence/absence matrix.
Metabarcoding data although gives us the number of reads for each BIN, does not mean we have a measure of abundance for that BIN. Detectability issues and biases must be well understood. Refer to this article for better understanding: https://onlinelibrary.wiley.com/doi/full/10.1111/1755-0998.13703
...
The chunk above now gave us a filtered taxmap object for each of the Arthropod Program's focal Orders (plus Diptera because I think its interesting).
...
# The 10 most abundant taxonomy will be visualized by default (parameter `topn=10`). 
pclass_fam <- ggbartax(obj=classtaxa_fam, facetNames="SEASON", topn=10) +
  xlab(NULL) +
  ylab("Presence (%)") +
